...
class UpcomingIndian extends Component {
		...
		loadMoviesByYear = (page) => {
				...
				axios.get('/3/movie/upcoming?api_key=65324ba8898442570ac397a61cfa7f22&region=IN&language=hi&sort_by=popularity.desc&with_release_type=3|2&original_language=hi&page=' + page)
        .then( response => {
            this.setState({
                posts : response.data
            });
            this.props.fetchMovies({ posts: response.data, 'type': 'UPCOMING_INDIAN' });
        }).catch( error => {
            console.log( error );
        });
		}
...
export default connect(null, mapDispatchToProps)(UpcomingIndian);